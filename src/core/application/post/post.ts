...
export class PostApplication {
  ...
  static async commitEdit({ compositePostId, post, postUrl, newFileAttachments }: TEditPostInput) {
    ...
    try {
      ...
      await LocalPostService.edit({ compositePostId, content: post.content, attachments });
      ...
      if (removedOriginalAttachmentUris.length > 0) {
        try {
          await FileApplication.commitDelete(removedOriginalAttachmentUris);
        } catch (cleanupError) {
      ...
      if (hasNewFiles) {
        try {
          await FileApplication.commitDelete(newFileUris);
        } catch (fileRollbackError) {
