...
export enum PostStreamTypes {
  ...
  // ============================================
  // The saved-posts feed renders from the local `bookmarks` table (see
  // `useBookmarksFeed`); only the combined ALL stream (seed) and the COLLECTION
  // stream (Followed Collections) are still fetched. Kind-filtered and
  // popularity bookmark streams were removed with the stream-cache feed.
