      ...
      <Container size="container" className="gap-10 px-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {FEATURES.map(({ key, image }) => (
            <article
              key={key}
              className="relative flex flex-col overflow-hidden rounded-md bg-card/80 p-6 shadow-sm backdrop-blur-sm md:max-xl:flex-row md:max-xl:items-start md:max-xl:gap-6"
            >
              <Image
                src={image}
                alt=""
                width={336}
                height={336}
                className="mb-6 size-32 md:max-xl:mb-0 md:max-xl:shrink-0 xl:mx-auto"
                aria-hidden
              />
