...
describe('groupPlainReposts', () => {
  ...
  it('throws for malformed composite post IDs', () => {
    const postIds = ['invalidNoColon', 'user2:repost2'];
    const detailsMap = new Map([
      ['invalidNoColon', createDetails('invalidNoColon', '', 1000)],
      ['user2:repost2', createDetails('user2:repost2', '', 2000)],
    ]);
    const relationshipsMap = new Map([
      ['invalidNoColon', createRelationships('invalidNoColon', 'pubky://original/pub/pubky.app/posts/post1')],
      ['user2:repost2', createRelationships('user2:repost2', 'pubky://original/pub/pubky.app/posts/post1')],
    ]);

    expect(() => groupPlainReposts(postIds, detailsMap, relationshipsMap, null)).toThrow('Invalid composite id');
  });
});
